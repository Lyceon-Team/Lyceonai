...
export interface ValidationResult {
  isCorrect: boolean;
  mode?: 'multiple_choice' | 'free_response' | 'mc' | 'fr';
...
export function toQuestionVM(apiQuestion: any): QuestionVM {
  const inferredType: 'mc' | 'fr' =
    apiQuestion.type === 'fr' ||
    apiQuestion.questionType === 'free_response' ||
    apiQuestion.question_type === 'free_response'
      ? 'fr'
      : 'mc';

  const questionType = inferredType === 'fr' ? 'free_response' : 'multiple_choice';
