...
export default function FullLengthReviewView({
  ...
  const totalCorrect = data.responses.filter(
  ...
  return (
    <Card className="bg-card/90 border-border/60">
      <CardHeader>
        ...
        <CardDescription>
          Runtime-backed review from
          `/api/full-length/sessions/:sessionId/review`. No inferred answers are
      ...
      <CardContent className="space-y-5">
        ...
        <Accordion type="multiple" className="space-y-3">
          {data.modules.map((module) => {
            ...
            const moduleCorrect = moduleResponses.filter(
              (response) => response.isCorrect === true,
            ).length;
                  ...
                  <div className="flex-1 text-left">
                    <div className="flex items-center justify-between gap-3 mb-2">
                      ...
                      <p className="text-sm font-semibold">
                        {moduleCorrect}/{moduleResponses.length || 0}
                      </p>
                    </div>
                  ...
                  {moduleResponses.length === 0 ? (
                    ...
                    <div className="space-y-3 pb-2">
                      {moduleResponses.map((response, index) => {
                        ...
                        return (
                          <div
                            key={`${module.id}-${response.questionId}-${index}`}
                          ...
                          >
                            ...
                            <p className="text-sm leading-relaxed whitespace-pre-wrap mb-3">
                              {question?.stem ||
                                "Question text unavailable in review payload."}
                            </p>
                            <div className="flex flex-wrap items-center gap-2 text-xs mb-2">
                              ...
                              {response.isCorrect !== null && (
                                <Badge
                                  variant={
                                    response.isCorrect ? "default" : "secondary"
                                  }
                                >
