-- ===========================================================================
-- RESTORE THE TABLE-DRIVEN EXAM WALK THAT A LATER-SORTING MIGRATION OVERWROTE
-- ===========================================================================
-- @spec [Doc 05E §3 Rule 4, §5, §6 INV-05E-06 / INV-05E-07; Doc 05E §7 L2-08..L2-15;
--        Doc 04B §9.4 (score_runs insert-once)] | @implemented [2026-09-26]
--
-- plain English: `public.execute_account_deletion_cascade` currently ships the WRONG body.
-- Two changes landed on two branches, each green on its own:
--
--   20260930100000_exam_child_tables.sql  (E9 commit 0, via `main`)
--       replaced the hardcoded nine-table exam block with a walk over
--       `public.exam_child_tables`, so the next exam table is a list row rather than an
--       edit to this function.
--   20261005000000_actor_id_integrity_sentinel.sql  (SCL-151, via `cleanup`)
--       rewrote the INV-05E-07 sentinel to check the actor_id VALUE, not just its nullity.
--
-- `20261005` sorts AFTER `20260930100000`, and it carried a full CREATE OR REPLACE built from
-- the body that predated E9. So the moment the two branches met, the pipeline's final body was
-- E9-less: the hardcoded exam block was back in all three places it appears (the hard_delete
-- walk, the sentinel's table list, the anonymize counts). `genesis-fresh-apply` caught it on the
-- merge commit — neither branch's own CI could, because neither branch had both changes.
--
-- This is the superseded-body class the working rules name: a change that replaces a function
-- body must be checked against the LAST migration defining it, and re-checked at merge time.
-- The re-check at merge time is the part that was missing.
--
-- WHAT THIS DOES: redefines the function as the UNION of the two changes — E9's
-- `exam_child_tables` walk (all three sites) plus SCL-151's value-checking sentinel. Nothing
-- else differs from either body; verified by diffing this body against both parents, which
-- yields exactly the sentinel hunk against E9 and exactly the three exam hunks against SCL-151.
--
-- WHY NO COVERAGE IS LOST where E9's sentinel list read `exam_child_tables.student_column` to
-- reach `test_sessions` and `score_runs`: both carry an `actor_id` column
-- (`20260930080000_exam_deletion_cascade.sql:105-106`) and a `student_id` column, so SCL-151's
-- catalog discovery finds them without a list. The discovered set is 9 tables, a strict superset
-- of the 7-table floor the sentinel refuses to run below.
--
-- WHAT THE REGRESSION DID AND DID NOT COST — measured, not assumed. The superseding hardcoded
-- block names exactly the same nine tables as the nine `exam_child_tables` rows (compared set
-- against set: no table missing, none extra), so no exam row was left behind by any deletion
-- that ran against it. What was lost is the property E9 bought: the next exam table is a list
-- row, and a cascade that walks the list picks it up. With the hardcoded block live, a tenth
-- list row would be deleted by nothing while `exam-deletion-cascade-gates.sql` L4 went red. So
-- this is a coverage-and-maintainability regression, not a data-retention one. Stated precisely
-- because an earlier note in this workstream put the hardcoded block's coverage at six of nine
-- tables and said `exam_runtime_outbox` survived; both were wrong.
--
-- A SECOND DEFECT, fixed in the same change: `scripts/ci/genesis-schema.expected.sql` was
-- carrying a function body PostgreSQL refuses to compile. That file is a `pg_dump` artifact and
-- is never hand-edited, but git merged it AS TEXT at `2a50cb2b` and kept BOTH sides' insertions
-- — E9's `FOR v_sentinel_tbl, ... IN` and SCL-151's `FOR v_sentinel_rec IN`, four FOR headers
-- against three LOOPs. Fed back to Postgres it yields `syntax error at or near
-- "v_sentinel_tbl"`. It failed safe here (a corrupt expected file makes the gate red, not
-- green), but a textual merge of a dump is capable of pinning a body no database ever had. The
-- snapshot is regenerated from the pipeline in this change, not edited.
--
-- NOT an edit to `20261005000000`: that migration is applied in production (owner report of
-- 2026-09-25), so its recorded text stands and the correction is a new migration.
--
-- trade-offs / edge cases: `CREATE OR REPLACE` only — no data change, no DDL, idempotent, and
-- safe to re-apply. A future change to this function must again find its LAST definer, which is
-- now this file.
-- ===========================================================================
