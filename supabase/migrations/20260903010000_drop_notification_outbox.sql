-- ---------------------------------------------------------------------------
-- Notifications rebuild — Migration B: drop the inert outbox.
-- LYCEON-MIGRATION-REVIEWED
--
-- @spec [contracts/notifications.contract.md §0 (supersedes the outbox contract);
--        Doc-01_V8 §40 account deletion cascade]
-- @implemented 2026-09-03
--
-- plain English: `notification_outbox` had one writer (a separate-round-trip insert in
-- guardian-routes, removed in the Step 2 deletion commit) and no reader anywhere. Migration A
-- replaces it with notification_events / notification_messages. This file drops the table
-- and corrects the FK-cascade inventory comment inside execute_account_deletion_cascade,
-- which named notification_outbox as one of the auto-CASCADE edges from profiles(id).
--
-- WHY A SEPARATE FILE. The owner applies this LAST, after Migration A is verified in
-- production. The two rows the table holds in production are test artifacts from
-- 2026-09-02 (both guardian_linked, both unprocessed, no reader) and are lost with it.
--
-- WHY THE WHOLE FUNCTION IS RECREATED. The comment lives inside the plpgsql body, and a
-- function body can only be changed by CREATE OR REPLACE. The body below is a verbatim copy
-- of 20260626010000_05e_anonymize_disposition.sql with ONE comment line changed; the
-- rebuild PR records a pg_dump diff of the function before and after showing exactly that.
-- No deletion behaviour changes.
--
-- IDEMPOTENT. DROP TABLE IF EXISTS; CREATE OR REPLACE FUNCTION.
--
-- NOT APPLIED BY THIS SESSION — the owner applies all SQL.
--
-- rollback:
--   re-run supabase/migrations/20260617000000_notification_outbox.sql (recreates the table,
--   empty) and re-run 20260626010000_05e_anonymize_disposition.sql (restores the comment).
-- ---------------------------------------------------------------------------
