...
const mockStripe = {
  ...
  checkout: {
    sessions: {
      create: vi
        .fn()
        .mockResolvedValue({ url: "http://checkout", id: "cs_123" }),
    },
  },
};
...
const mockGetProfileStripeCustomerId = vi.fn();
const mockSetProfileStripeCustomerId = vi.fn();

vi.mock("../lib/account", () => ({
  getPrimaryGuardianLink: (...args: unknown[]) =>
    mockGetPrimaryGuardianLink(...args),
  getEntitlementForProfile: vi.fn().mockResolvedValue(null),
  getProfileStripeCustomerId: (...args: unknown[]) =>
    mockGetProfileStripeCustomerId(...args),
  setProfileStripeCustomerId: (...args: unknown[]) =>
    mockSetProfileStripeCustomerId(...args),
...
describe("Billing checkout (guardian)", () => {
  ...
  it("guardian checkout targets linked student profile in metadata", async () => {
    mockGetPrimaryGuardianLink.mockResolvedValue({
      student_user_id: "student_123",
      account_id: "student_acc_123",
    });
    mockGetProfileStripeCustomerId.mockResolvedValue(null);
    mockSetProfileStripeCustomerId.mockResolvedValue(undefined);
    ...
    expect(mockSetProfileStripeCustomerId).toHaveBeenCalledWith(
      "student_123",
      "cus_123",
    );
  });
