...
describe("Billing Truth", () => {
  beforeEach(() => {
    ...
    (accountLib.mapStripeStatusToEntitlement as any).mockReturnValue({
      tier: "premium",
      status: "active",
    });
  });
    ...
    mockGetUncachableStripeClient.mockResolvedValue({
      webhooks: {
        constructEvent: vi.fn().mockReturnValue({
          ...
          id: mockEventId,
          data: {
            object: { id: "sub_123", metadata: { profile_id: "prof_123" } },
          },
        }),
      },
  ...
  it("guardian-paid checkout applies entitlement to selected student, not guardian", async () => {
    ...
    const mockRetrieve = vi.fn().mockResolvedValue({
      ...
      status: "active",
      customer: "cus_123",
      metadata: { profile_id: "student_prof_123" },
    });
    ...
    expect(accountLib.upsertEntitlement).toHaveBeenCalledWith(
      "student_prof_123",
      expect.any(Object),
    );
  });
  ...
  it("runtime access gate reads canonical student entitlement, not legacy profile billing fields", async () => {
    ...
    (
      accountLib.resolveLinkedPairPremiumAccessForStudent as any
    ).mockResolvedValue({
    ...
    const access =
      await accountLib.resolveLinkedPairPremiumAccessForStudent("student_123");
