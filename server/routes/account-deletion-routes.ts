...
type RecoveryResult =
  ...
  | {
      ok: false;
      code: "INVALID_OR_EXPIRED" | "EMAIL_RECLAIMED" | "ERROR";
      message: string;
    };
...
export async function performRecovery(
  ...
  const { data, error } = await admin.rpc("restore_account_deletion", {
  ...
  if (error) {
    if (error.code === "23505") {
      return {
        ...
        message: "Email is no longer available for restoration",
      };
    }
...
type InAppCancelResult =
  ...
  | {
      ok: false;
      code: "NO_PENDING" | "EMAIL_RECLAIMED" | "ERROR";
      message: string;
    };
...
export async function performInAppCancel(
  admin: DeletionAdminClient,
  profileId: string,
): Promise<InAppCancelResult> {
  ...
  if (error) {
    if (error.code === "23505") {
      return {
        ...
        code: "EMAIL_RECLAIMED",
        message:
          "Email is no longer available; account is still scheduled for deletion",
      };
    }
    return { ok: false, code: "ERROR", message: error.message };
  }
  ...
  try {
    ...
    await sendEmail({
      ...
      html:
        ...
        `<p>If you have a paid subscription, it stays active until <strong>${when}</strong> and is then cancelled.</p>` +
    ...
    try {
      ...
      if (isDeletionLifecycleV2Enabled()) {
        ...
        if ("error" in result) {
          logger.error(
            "DELETION",
            "request_v2_error",
            "Failed to process deletion request (v2)",
        ...
        logger.info(
          "DELETION",
          "requested",
          "User requested account deletion (v2 soft-delete)",
          { userId, requestId },
        );
        return res.json({
          ok: true,
      ...
      if (findError) {
        logger.error(
          "DELETION",
          "fetch",
          "Failed to check existing deletion requests",
          { userId, error: findError.message, requestId },
        );
        return res
          .status(500)
          .json({ error: "Failed to process deletion request", requestId });
      }
      ...
      if (error) {
        logger.error(
          "DELETION",
          "insert_error",
          "Failed to insert deletion request",
          { userId, error: error.message, requestId },
        );
        return res
          .status(500)
          .json({ error: "Failed to queue account for deletion", requestId });
      ...
      logger.info("DELETION", "requested", "User requested account deletion", {
        userId,
        requestId,
      });
    ...
    try {
      ...
      if (pendingError) {
        logger.error(
          "DELETION",
          "cancel_fetch_error",
          "Failed to load deletion request for cancellation",
          { userId, error: pendingError.message, requestId },
        );
        return res
          .status(500)
          .json({ error: "Failed to cancel deletion request", requestId });
      }
      ...
      if (isDeletionLifecycleV2Enabled()) {
        ...
        if (!result.ok) {
          if (result.code === "EMAIL_RECLAIMED") {
            logger.warn(
              "DELETION",
              "cancel_email_reclaimed",
              "In-app cancel rolled back: email reclaimed during grace (request still pending)",
              { userId, requestId },
            );
            return res.status(409).json({
              error:
                "Your email address is no longer available, so we could not restore your account. It is still scheduled for deletion — please contact support to recover it.",
              code: "EMAIL_RECLAIMED",
              requestId,
            });
          }
          ...
          logger.error(
            ...
            { userId, error: result.message, requestId },
          );
          return res
            .status(500)
            .json({ error: "Failed to cancel deletion request", requestId });
      ...
      logger.info("DELETION", "cancelled", "User cancelled deletion request", {
        userId,
        requestId,
      });
      res.json({
  ...
  try {
    ...
    if (!result.ok) {
      if (result.code === "ERROR") {
        logger.error("DELETION", "recover_error", "Failed to restore account", {
          error: result.message,
          requestId,
        });
        return res
          .status(500)
          .json({ error: "Failed to restore account", requestId });
      }
      const status = result.code === "EMAIL_RECLAIMED" ? 409 : 404;
      return res
        .status(status)
        .json({ error: result.message, code: result.code, requestId });
    }

    logger.info("DELETION", "recovered", "Account restored via recovery link", {
    ...
    logger.error(
      "DELETION",
      "recover_unhandled",
      "Unhandled error restoring account",
      {
        error: err instanceof Error ? err.message : String(err),
        requestId,
      },
