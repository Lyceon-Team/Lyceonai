  ...
  try {
    ...
    if (error) {
      logger.error(
        "RATE_LIMIT",
        "check",
        "Failed to check rate limit - table may be missing, failing closed",
        {
          error,
          key,
      ...
      throw new Error(
        `Rate limit check failed: ${error.message}. Ensure guardian_link_audit table exists.`,
      );
    }
    ...
    if (!allowed) {
      logger.warn(
        "RATE_LIMIT",
        "exceeded",
        "Rate limit exceeded for guardian",
        {
          key,
          currentCount,
          maxAttempts,
      ...
      if (insertError) {
        logger.error(
          "RATE_LIMIT",
          "audit_insert",
          "Failed to insert rate limit audit - failing closed",
          {
            error: insertError,
            key,
            request_id: requestId,
        ...
        throw new Error(
          `Rate limit audit insert failed: ${insertError.message}`,
        );
      }
    }
  ...
  return async function durableRateLimiterMiddleware(
    ...
    try {
      ...
      logger.error(
        "RATE_LIMIT",
        "middleware",
        "Rate limit infrastructure failure - blocking request",
        {
          err,
          key,
          request_id: requestId,
