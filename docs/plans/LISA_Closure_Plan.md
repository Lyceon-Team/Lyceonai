...
| W3-3 | All students get US crisis resources | Production: `country_code = SG` returns Samaritans of Singapore 1767; `XX` and null return the no-number fallback; no 988. Write path proven by the Postgres-backed webhook test (`sg` billing country → `country_code = 'SG'`) | 09-26 |
