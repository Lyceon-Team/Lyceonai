...
| **W4-7** | No record of whether LISA was used on an attempt | BUILT 09-26 (branch `…-w4-7-assisted`) — the flag is the spec's existing `review_error_attempts.used_tutor` (Doc 02B §16, CR-02B-16: telemetry only), which the writer hard-coded `false` since 20260921000000; no new `assisted` column. Migration `20261005000000_review_attempt_used_tutor.sql`: the `review_item_resolve()` trigger sets it from `review_item_used_tutor(student, item)` — true when the student sent ≥1 message (`role='student'`, `content_kind='message'`) on that item's review conversation before submit. A lookup error records `false` with a content-free WARNING and never fails the submit. Nothing reads it (gate G22 for SQL; `tests/ci/used-tutor-no-reader.contract.test.ts` for app code). **Review only:** practice has no LISA and no conversation, so every practice attempt to date is truthfully unassisted; its column lands with LISA-in-practice, and nothing is lost by waiting. **Proof owed after apply:** a review attempt where the student messaged LISA shows `used_tutor = true`, one without shows `false` | CC |
| **W4-9** | SCL amendments for Doc 03 §4.6 and §21.3 | DONE 09-26 — PR #906 (merged to `cleanup`): **SCL-171** records the §4.6 resource table as built (youth lines, two lanes, `GB` key) and the no-number fallback with both lane texts as shipped, numbers unchanged; **SCL-172** records the §21.3 review procedure as DEFERRED POST-LAUNCH (owner decision 09-26): what exists (detection, alerting, the SLA sweep, the dashboard, dispositions) and what does not (the written procedure). Both PROPOSED; the spec edits are Karl's | Karl |
