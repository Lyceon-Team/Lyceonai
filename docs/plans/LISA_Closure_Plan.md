...
| **W3-3** | All students get US crisis resources | BUILT 09-25 (branch `…-w3-3-country`) — the Stripe grant path records the billing country the INV-03-08 gate approved (`Customer.address.country`) on each entitled student's `profiles.country_code`, on every grant; crisis and safeguarding resources resolve from it (`server/services/crisis-resources.ts`). **Ruling 2026-09-25:** unknown or unsupported country gets a named response that gives **no number** — `UNKNOWN_COUNTRY_CRISIS_RESPONSE` / `UNKNOWN_COUNTRY_SAFEGUARDING_RESPONSE`: local emergency number plus a trusted adult (replaces the US default the build first shipped; a student outside the US calling 988 gets nothing). Logs WARN `crisis_country_defaulted` (student id, digested; no content). **Ruling 2026-09-25 — resource table:** the code's youth lines for CA, IE, AU, NZ (Kids Help Phone, Childline Ireland, Kids Helpline, Youthline) are right for 15–18-year-olds; Doc 03 §4.6's adult lines are stale → **SCL amendment owed to the spec, no code change**. **Accepted lag:** a move *within* the seven reaches `country_code` at the next grant/renewal, up to one billing period (a move *out* cancels at period end, SCL-047); writing on `customer.updated` is a post-launch follow-up. Backfill: `docs/plans/W3-3_country_backfill.sql` + `scripts/ops/backfill-country-code.ts` (prints SQL, writes nothing). **Proof owed (closes W3-3):** (1) a student record with a non-US country set through the real path; (2) a crisis turn from that student showing that country's resources; (3) the backfill Q0/Q2 output — how many got a country, how many remain null | CC + Karl |
