...
| W3-3 | All students get US crisis resources | (1) Resolver proven in production: `SG` → Samaritans of Singapore 1767; `XX`/null → no-number fallback; no 988. (2) Write path proven by the Postgres-backed webhook test (`sg` billing country → `country_code = 'SG'`). (3) Backfill: 2 entitled students set to `US` from their Stripe billing addresses — one the payer's own customer, one through a guardian payer. 3 customers skipped: no address, no active entitlement | 09-26 |
| W4-4 | LISA panel overlaps Desmos; panel is opt-in | Three-panel layout in production; Desmos fully usable; LISA open on load with the opener | 09-26 |
| W4-7 | No record of whether LISA was used on an attempt | Proven in production (migration `20261005000000`): 07:16:39 `used_tutor = true` after messaging LISA; 07:17:07 `used_tutor = false` without. Existing Doc 02B §16 column filled; no new column | 09-26 |
| W4-9 | SCL amendments | SCL-171 and SCL-172 merged in #906 | 09-26 |
| W2-2b | Legacy cases breaching hourly | All 11 cases resolved via the admin dashboard; zero open. Dashboard, claim flow and CSRF all exercised for the first time | 09-26 |
...
| **W2-2b** | Legacy cases breaching hourly | **CLOSED 09-26** — all 11 cases resolved via the admin dashboard; zero open; see the Closed table. (Opened as three identified cases; eleven were open when resolved) | Karl |
...
**CLOSED 09-26** — resolved through the admin dashboard, not by the SQL below; kept for the record.
...
| **W3-3** | All students get US crisis resources | **CLOSED 09-26** — proven in production, including the backfill; see the Closed table. Built in PR #900: the Stripe grant path records the billing country the INV-03-08 gate approved on `profiles.country_code`; crisis and safeguarding resources resolve from it (`server/services/crisis-resources.ts`). **Ruling 2026-09-25:** unknown or unsupported country gets the no-number response (`UNKNOWN_COUNTRY_CRISIS_RESPONSE` / `UNKNOWN_COUNTRY_SAFEGUARDING_RESPONSE`: local emergency number plus a trusted adult), with WARN `crisis_country_defaulted`; wording as merged in #900. **Ruling 2026-09-25 — resource table:** the code's youth lines for CA, IE, AU, NZ are right for 15–18-year-olds; Doc 03 §4.6's adult lines are stale → **SCL amendment owed to the spec, no code change**. **Accepted lag:** a move *within* the seven reaches `country_code` at the next grant/renewal; writing on `customer.updated` is a post-launch follow-up. Backfill tooling: `docs/plans/W3-3_country_backfill.sql` + `scripts/ops/backfill-country-code.ts` | CC + Karl |
...
| **W3-11** | LISA's per-request access check has no country clause | OPEN — Doc 03 §12 lists billing country IN Tier-1 as the fourth condition on every LISA request; `entitlement_active` checks tier and status only. Tier-1 is enforced at every GRANT (INV-03-08 gate) and on egress (SCL-047: leaving Tier-1 cancels at period end), so an out-of-list student can hold access until period end, and a comped/manual entitlement is never checked. Such a student has no configured crisis resources and gets the no-number response. **Ruling 2026-09-25: accepted, POST-LAUNCH, not launch-blocking** — every grant is gated and leaving the seven cancels at period end, so the exposure is a manual grant or one billing period of lag | Karl |
...
| **W4-4** | LISA always open in review, three-panel layout | **CLOSED 09-26** — three-panel layout in production; see the Closed table. Built in PR #895 (client + one read-only server filter): LISA open on every review question with an opener ("Need help with this question?"), which is presentation only. **No conversation on load:** the panel finds the item's existing thread with `GET /conversations?source_session_item_id=` and creates one only on the first real message. Layout: ≥1446px question \| Desmos \| LISA; 1024–1445px Desmos expands over LISA's column (owner ruling 09-25; LISA stays mounted); <1024px one column — question, LISA, calculator. Hide LISA hides it for the current question; it returns on the next. Practice untouched (LISA in practice is separate, later work). Proven 09-26 (Closed table): three-panel layout, Desmos fully usable, LISA open on load with the opener. Zero `POST /conversations` before the first send is pinned by `ScopedTutorPanel.contract.test.tsx`, not separately observed in production | CC |
...
| **W4-7** | No record of whether LISA was used on an attempt | **CLOSED 09-26** — proven in production; see the Closed table. Built in PR #914 — the flag is the spec's existing `review_error_attempts.used_tutor` (Doc 02B §16, CR-02B-16: telemetry only), which the writer hard-coded `false` since 20260921000000; no new `assisted` column. Migration `20261005000000_review_attempt_used_tutor.sql`: the `review_item_resolve()` trigger sets it from `review_item_used_tutor(student, item)` — true when the student sent ≥1 message (`role='student'`, `content_kind='message'`) on that item's review conversation before submit. A lookup error records `false` with a content-free WARNING and never fails the submit. Nothing reads it (gate G22 for SQL; `tests/ci/used-tutor-no-reader.contract.test.ts` for app code). **Review only:** practice has no LISA and no conversation, so every practice attempt to date is truthfully unassisted; its column lands with LISA-in-practice, and nothing is lost by waiting. | CC |
| **W4-9** | SCL amendments for Doc 03 §4.6 and §21.3 | **CLOSED 09-26** — PR #906 (merged to `cleanup`): **SCL-171** records the §4.6 resource table as built (youth lines, two lanes, `GB` key) and the no-number fallback with both lane texts as shipped, numbers unchanged; **SCL-172** records the §21.3 review procedure as DEFERRED POST-LAUNCH (owner decision 09-26): what exists (detection, alerting, the SLA sweep, the dashboard, dispositions) and what does not (the written procedure). Both PROPOSED; the spec edits are Karl's | Karl |
