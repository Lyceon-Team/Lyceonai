...
| 31 | §9.2, §13 | **An activity unit is retrieval, never a skip.** The adapters select activity with `status = 'answered'`, not `answered_at IS NOT NULL`. Those read as synonyms and are not: a skipped item also carries a non-null `answered_at` — review has two such rows in production today, and review's handoff puts practice skips into the queue as well. On the old predicate a student could clear a block by skipping through it and §13 would report the work done. `answered_at` remains the unit's `occurred_at`: it was the wrong FILTER, not the wrong timestamp. **That last sentence is SUPERSEDED by item 34** — the filter was one half of the defect and the timestamp was the other. Changed before practice had any skips, because the first one turns this from a latent defect into silent bad data with no error to notice. | **RULING 2026-09-22** (review handoff H2) | `server/services/calendar/adapters/practice.ts`; `tests/ci/calendar.activity-units.skip.ci.test.ts`, plant recorded in the PR |
...
| 34 | §9.1, §9.2, §9.3, §22.4 | **A unit is dated by `occurred_at`, the column the schema guarantees — not by `answered_at`.** Both adapters now SELECT, window and date on `occurred_at`. **Supersedes** the 2026-09-17 mapping recorded in `allocate.ts` ("`occurred_at` is `answered_at` for both real engines"), the closing sentence of item 31, and review handoff **H2** on which timestamp the seam uses; the H2 rule that a skip is not retrieval is untouched and still stands. Reasoning: `psi_resolved_requires_occurred_at` and `rsi_resolved_requires_occurred_at` are the same CHECK on each table — `status <> ALL (ARRAY['answered','skipped']) OR occurred_at IS NOT NULL` — so every row the contract can return HAS an `occurred_at`, while `answered_at` is plain nullable `timestamptz` with nothing enforcing it. The two agree on every resolved row in production today (owner's count: 156 of 156) because `submitPracticeAnswer`, `submitReviewAnswer` and both skip paths write them from one `now` — a property of today's WRITERS, not of the schema. A resolved row that ever lands without `answered_at` drops out of the window in silence and is reported as "the student did nothing today": the same failure mode item 31 closed, pointing the other way. It is also the column `trg_review_item_resolve` feeds `review_error_attempts` from, so a calendar unit and a mastery event are now dated by the same instant rather than by two columns that merely agree. Review's adapter shipped with a note saying `occurred_at` was the stronger column and that changing BOTH adapters was a contract decision rather than that change's to make; this is that decision. | **RULING 2026-09-22** | `adapters/types.ts` (the contract rule), `adapters/practice.ts`, `adapters/review.ts`, `packages/shared/src/calendar/allocate.ts`; plant in `tests/ci/calendar.activity-units.skip.ci.test.ts` (fake client) and `tests/ci/calendar.launch-contract.review.test.ts` (real engine, real SQL, real constraint) |
| 35 | §9.1, §9.2, §9.3 | **The review create seam carries no origin and no platform.** The adapter calls `startOrReplayReviewSession` DIRECTLY — not `POST /api/review/sessions` — because the concurrent-session cap, the idempotency replay and the client-instance binding all live in that function, and a second entry point would be a second contract of which only one would ever get fixed. The argument object is exactly `{ studentId, actorId, poolSpec, clientInstanceId, idempotencyKey, targetCount }`. **No `source_origin`** — the signature takes none; review records what it needs from the session row. **No `platform`** — the signature takes none either, and review sets `platform: 'web'` on the row itself, so one passed from the calendar would be silently dropped, which is worse than not passing it. **`poolSpec` is `{ mode: "queue" }`**, a bare discriminant with no fields: a calendar review block means "the work you owe", which is the student's open queue in `review_due_by_date` order — the same queue the generator sized the block against. `filter` mode would be the calendar inventing a selection the plan never made; `session` mode belongs to "review this test", a different request. **`idempotencyKey` is forwarded UNCHANGED** (§9.2's rule, applied to the second engine): the calendar owns the key format, the engine owns what a repeated key means. **`targetCount` is THIS launch's size**, not the block target (§9.1). **Activity timestamp: `occurred_at`, per item 34.** **Activity predicate: `status = 'answered'` for BOTH engines** — never a nullness test on a timestamp. A SKIPPED item is resolved, so the CHECK guarantees it a timestamp exactly as it guarantees an answered one, and a skip is not retrieval; the two adapters use the identical predicate deliberately, so neither can drift into counting skips as work. **The due-by-date input reads `review_schedule.queued_at`, joined to `servable_questions`** (item 32), and reads neither `review_session_items` nor `review_sessions`. | **RULING 2026-09-22** | `server/services/calendar/adapters/review.ts`; `server/routes/review-canonical.ts` (the export); `tests/ci/calendar.launch-contract.review.test.ts` asserts the key set exactly |
| 36 | §4 | **The seam table: exactly what the calendar reads from review.** THREE relations, and nothing else. Anything not on this list is out of the seam, and adding a column here is a seam change that belongs in this record. — **`review_schedule`** (`student_id`, `status`, `queued_at`), read by `calendar_build_plan_input` in SQL, joined to `servable_questions`, for `review_due_by_date`: how much is owed and when it came due. · **`review_session_items`**, answered rows only (`id`, `question_section`, `question_domain`, `occurred_at`, `status`), read by `adapters/review.ts` `activityUnits`, for one activity unit per answered item on the local date. · **`review_sessions`** (`status`), read by `adapters/review.ts` `progress`, for the §13 lifecycle — `created` and `active` both mean active, plus `completed` and `abandoned`. The calendar **writes nothing** in the review schema. Its only mutation into review is the create call in item 35, which review performs on its own tables. | **RULING 2026-09-22** | `supabase/migrations/20260925000000` (`calendar_build_plan_input`); `server/services/calendar/adapters/review.ts` |
| 37 | G-08-03 | **"SM-2 writer live" becomes "queue writer live."** The old wording named an algorithm the seam does not depend on. What the calendar needs is that SOMETHING keeps `review_schedule` current — the practice and review triggers that enqueue and dequeue items, in production since **2026-09-21**. Whether the interval schedule behind them is SM-2 or anything else is review's business and invisible across the seam; naming SM-2 in a calendar gate made a calendar gate hostage to a review implementation detail. **The gate closes when both hold:** (1) the review launch contract test passes against the REAL engine — not the stub, not a mock — creating and replaying a real session and distinguishing answered from skipped items; and (2) `review` is present in `enabled_block_types`. Both are satisfied by PR #825: the real-engine half is PG-backed and runs behind `vitest-summary-gate.mjs`, which treats a skipped test as red, and `20260928000000` sets `enabled_block_types` to `["practice","review"]` as the last commit. **G-08-03 therefore closes on #825 merging and the owner applying `20260928000000`** — it is not left open. `full_length` stays off: its adapter is still a fail-open stub, its launch control reads "Coming soon" and never calls launch, and the builder nulls `full_length_weekday` and the exam facts while the flag is absent. | **RULING 2026-09-22** (review handoff G-08-03) | `supabase/migrations/20260928000000_calendar_enable_review_block_type.sql`; `tests/ci/calendar.launch-contract.review.test.ts` (half B); `.github/workflows/ci.yml` (the gated step); gates C-07, Z-45…Z-47 |
| 38 | §9.1, §15.1 | **`next` is the adapter's to give, on BOTH branches — and the calendar resumes DIRECTLY into the running session.** Two rulings in one row, because they are the same decision seen from each side. **(a) Ownership.** The launch service's resume branch (§15.1 step 3) built `/practice/session/<id>` itself, for every engine, while `create` asked the adapter. So a student resuming a live REVIEW block was sent to practice's page carrying a review session id — `GET /api/practice/sessions/8f1ccdc5…/state 404`, production 2026-09-22, dep `dpl_HzcSpFbn8J58G7rAonUzsNRATch8`, four times in eighty seconds. The FIRST launch of any block always worked, because that one goes through `create`; every launch after it took the broken branch, which is why this survived a contract suite that covered create, replay, activity units, progress and launch size. **Correcting the string alone would have fixed review and left the next engine free to repeat it**, so the contract gains `resumeHref(sessionId)` and the service is left with nothing to build a path from: a new engine supplies its own route or it does not compile. The resume branch also now asks the adapter for `latest.engine` — the engine that owns the session id — rather than the block's current type, which differ when a day was edited after a launch. **(b) Resume semantics.** The calendar lands the student IN the running session rather than bouncing to `/review` to pick from a list. Review's own page offers a choice because it is a hub a student browses; a calendar block is a specific instruction, and handing back a decision the plan already made would be the calendar forgetting what it is. **This divergence from review's landing page is deliberate — it is not an inconsistency to be tidied away later.** | **RULING 2026-09-23** | `server/services/calendar/adapters/types.ts` (`resumeHref` on the §9.1 contract); `adapters/practice.ts`, `adapters/review.ts`, `adapters/stub.ts`; `server/services/calendar/launch-service.ts`; landing assertions in `tests/ci/calendar.launch-contract.practice.ci.test.ts` and `…review.test.ts` via `tests/helpers/launch-landing.ts`; `tests/ci/calendar.launch-service.test.ts` |
