...
| ~~**`entitlements_profile_id_unique` is an index in prod and a table constraint in genesis**~~ **RESOLVED 2026-08-27** | `PCE §2.1`; PR #659 | Genesis was corrected toward production — it now declares `CREATE UNIQUE INDEX`, verified byte-identical to production's `indexdef` by a fresh apply of all migrations. Kept here struck through rather than deleted because the `pg_constraint`-only reading that this drift produced is what got the Phase 1 brief wrong, and that lesson outlives the drift. |
| **The repo and the production migration ledger disagree** | `supabase_migrations.schema_migrations` read 2026-08-27; PR #666 | 58 migrations in the repo, **16** in the ledger, **42 unrecorded**; the ledger's newest entry is `20260624020000` (2026-06-24). The unrecorded DDL is nonetheless **live** — `20260809000000`'s `entitlements_profile_id_unique` index exists in production while that migration is absent from the ledger — so migrations since June have been applied out-of-band without being recorded. Consequence: the ledger cannot be used to answer "has this been applied?", and any tool that trusts it (including Supabase's own migration tooling) will mis-sequence. **Owner-ruled closed 2026-08-27; recorded here once and not raised again in phase reports.** |
