---
document: Community Guidelines
version: 2.0
effective_date: 2026-09-11
supersedes: 1.0 (2025-12-22)
---

# **LYCEON Community Guidelines**

**Version 2.0 · Effective 11 September 2026**

LYCEON is a learning environment built on respect, safety, and trust. These Community Guidelines explain how users are expected to behave when using LYCEON.

These guidelines apply to all interactions on the platform, including AI chat and tutoring features, messages, feedback, or submissions, and any current or future community, leaderboard, or social features.

By using LYCEON, you agree to follow these guidelines.

---

## **1. Be Respectful and Authentic**

Treat others with respect and honesty.

You must not:

* Harass, bully, threaten, or intimidate others
* Use hateful, discriminatory, or abusive language
* Target individuals or groups based on personal characteristics
* Impersonate other users, LYCEON employees, tutors, moderators, or representatives
* Misrepresent your identity or role on the platform

LYCEON is for learning, not deception or harassment.

---

## **2. Keep Content Appropriate and Safe**

All content and interactions must be appropriate for a student learning environment.

You must not:

* Share explicit, sexual, or violent content
* Promote self-harm or dangerous behavior
* Post content that is intentionally misleading or harmful
* Share real-world personal contact information, including phone numbers, home addresses, social media handles, or messaging app usernames

Do not overshare personal information in any public or AI-facing areas of the platform.

---

## **3. Protect Your Link Code**

Your account settings display a **link code** that lets a parent or guardian connect to your account and see your progress data.

**Treat it like a password.**

You must not:

* Post your link code publicly
* Share it in any community, chat, or social feature
* Share it with anyone other than a parent or guardian you intend to have access

Your link code expires after 24 hours and stops working once used. If you have shared it with someone you did not intend to, generate a new one immediately from your account settings.

---

## **4. Use AI Responsibly**

LYCEON uses AI to support learning, not to replace judgment.

You must not:

* Attempt to bypass or manipulate AI safety systems
* Use prompt engineering to generate disallowed or unsafe content
* Encourage the AI to produce harmful, deceptive, or inappropriate responses
* Treat AI output as authoritative without critical thinking

If you encounter unsafe, misleading, or inappropriate AI behavior, report it.

---

## **5. No Cheating or Academic Misuse**

You must not:

* Use LYCEON during live, official, or proctored exams
* Use LYCEON to complete assignments intended to reflect your own work
* Share answers or explanations to gain unfair academic advantage

LYCEON exists to support learning, not to undermine academic integrity.

---

## **6. Protect the Platform**

You must not:

* Scrape, copy, or redistribute LYCEON content without permission
* Reverse engineer, exploit, or interfere with platform systems
* Attempt to bypass safeguards, limits, or monitoring
* Use LYCEON in ways that disrupt service for others

---

## **7. Report Issues**

If you encounter harassment or abuse, unsafe or inappropriate content, misuse of AI tools, or behavior that violates these guidelines, report it through the platform or designated support channels.

Reporting helps keep LYCEON safe and effective for everyone.

---

## **8. Enforcement and Monitoring**

Violations of these Community Guidelines may result in content removal, warnings, temporary suspension, or permanent account termination.

To help maintain a safe learning environment, LYCEON may use automated systems and human review to monitor activity and enforce these guidelines.

LYCEON reserves the right to enforce these rules at its discretion to protect users and the platform.

---

## **9. Updates to These Guidelines**

LYCEON may update these Community Guidelines as the platform evolves. Each version carries a version number and effective date.

Continued use of LYCEON after updates take effect constitutes acceptance of the revised guidelines.

---

## **10. Contact**

**support@lyceon.ai**

LYCEON AI
