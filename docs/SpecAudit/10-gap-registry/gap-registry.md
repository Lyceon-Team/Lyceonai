...
| GAP-SP-25 | Doc 05B §5.3/§6.6 name the guardian-access tables `guardian_student_links(guardian_id, linked_student_id, link_active)` + `student_entitlements(student_id, active)`; the genesis identity model (Doc 01 V8) names `guardian_links(guardian_profile_id, student_profile_id, status)` + `entitlements(profile_id, status)`. 05B's guardian RLS uses the genesis canonical tables + the repo's canonical "entitlement active" definition (`status IN ('active','past_due')`, matching genesis `idx_entitlements_active`). Reconcile the 05B spec table/column names owner-side (`docs/Spec` read-only). Also: 05B §7 KPI UNIONs name `practice_attempts_v0`/`test_session_answers` — same stale-source class as SP-22; 05B KPI reads `practice_session_items[answered]`+`review_error_attempts` (test branch deferred to WS-4) | 05B (2026-06-13) | OPEN — code correct; spec names stale |
