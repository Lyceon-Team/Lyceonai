...
| GAP-SP-21 | `question_id` TYPE mismatch across the WS-2↔WS-3 seam: genesis `public.questions.id` is **TEXT** (`^SAT(M\|RW)[12][A-Z0-9]{6}$`), but the frozen seam §2 R1, Doc 05A `apply_mastery_event(p_question_id uuid)`, and B-WS3-1 `mastery_event_audit_log.question_id uuid` all type it **uuid**. B-WS2-1 answer tables use `question_id text` (FK to `questions.id`) — the canonical truth. The seam's `uuid` typing must be reconciled to TEXT (or a uuid surrogate added to `questions`) BEFORE Lane C wires the `apply_mastery_event` call / `canonical_mastery_events` view. `event_id` (answer-row PK uuid) is unaffected | B-WS2-1 (2026-06-10) | OPEN — blocks Lane C wiring |
