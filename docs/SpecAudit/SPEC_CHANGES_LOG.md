...
SCL-167 | 2026-09-25 | Doc 05F §19's test matrix specifies 403 for a guardian with no link and 403 for a link to an unentitled student; the ruled and shipped behaviour is 404 and 402, which §16 already describes — the document contradicts itself | PROPOSED
...
Change: §19's Routes row reads **404** for a guardian with no link and **402** for a link to an unentitled student, matching §16 and the resolver.
...
IS: the same row with `403` replaced by `404` for no-link and by `402` for link-but-unentitled. Everything else in the row is correct and stays.
...
SCL-168 | 2026-09-25 | `entitlement_features.required_age_minimum` is populated on every feature row and is read by nothing: `canAccessFeature` selects `required_tier, enabled` only, so the platform's age gate has never been evaluated | PROPOSED
...
Owner action: rule (a) or (b) and amend Doc 01 §27 accordingly. Out of the calendar's scope either way; no behaviour change here.
