...
SCL-090 | 2026-09-17 | No locked document owns the do-not-contact list: the deletion flow can promise "never contact me again", the build now keeps that promise indefinitely from a hashed record, and a permanent rule about a minor's personal data belongs in the spec rather than in a repo contract alone | PROPOSED
...
Change: add to Doc 01 V8 §40 (account deletion lifecycle) a subsection, and a row to the §5.1 retention-tier table. Proposed §40 text: `A deletion request may ask that the address never be contacted again. Where it does, the platform retains a keyed hash of that address — never the address — for as long as the suppression stands, and consults it before every product or marketing send. Transactional mail belonging to the deletion itself, notably the completion notice, is not subject to it. Registration is not subject to it either: a suppressed address may be registered again, and refusing it would make the signup form disclose that the address was once deleted.` Proposed §5.1 row: `| Do-not-contact records (keyed hash of the address; no other field) | While the suppression stands | The promise has no end date; the record is what makes it enforceable |`.
...
IS: `supabase/migrations/20260917110000_deletion_suppression.sql` creates `public.deletion_suppression` (log_id, address_hash — no timestamp, no identity-graph key, RLS on with zero policies, service_role SELECT only) plus the gated writer and the read used by the send path; `server/lib/deletion-suppression.ts` holds the HMAC and the three outcomes (`suppressed` / `not_suppressed` / `unknown`, the last deferring rather than sending); `server/lib/notifications/dispatch.ts` consults it before every product send; `contracts/notifications.contract.md` §11A states the whole rule with its falsifiers.
Rationale: the platform can only keep "never contact me again" by retaining something derived from the address after the address is gone, and what it retains is a permanent record about a person — usually a minor — held indefinitely. Under the register's own test that is "silence that must become spec text": the outcome is that the owner amends a document, not that the repo does something. It is also the one member of the evidence bundle exempt from SCL-085's 24-month strip, which is a retention rule the §5.1 table should carry rather than leaving it to a migration comment.
Why this surfaced now: Phase 2 of the owner brief of 2026-09-17 built it; the brief specified the mechanism and the boundaries but no locked document records either.
Owner action: amend Doc 01 V8 §40 and the §5.1 table with the text above. Then rule on the open question below, which the build could not decide by itself.
OPEN QUESTION FOR THE OWNER AND COUNSEL — a returning student is silently unreachable. Suppression persists while it stands, and registration deliberately does not consult it, so a student who deletes with suppression and then signs up again with the same address has a live account that receives no product notifications, with nothing in the product to tell them so or to clear it. Three readings: (a) the suppression stands until the person withdraws it, and the new account needs a visible "we have you on a do-not-contact list" control — most protective of the original wish, and the current behaviour; (b) completing a fresh signup with that address is itself the withdrawal, so registration clears the record — most protective of the returning student, but it lets anyone who can receive mail at an address undo a suppression; (c) the suppression is scoped to the deleted account rather than to the address, which is not implementable, since the deleted account is precisely what no longer exists to scope to. The build implements (a) because it is the reading that cannot be wrong about consent, and the choice is recorded here rather than made silently. Switching to (b) is one call in the registration path and one new gated function.
Build artifact: `supabase/migrations/20260917110000_deletion_suppression.sql`, `server/lib/deletion-suppression.ts`, `server/lib/notifications/dispatch.ts`, `contracts/notifications.contract.md` §11A, `infra/secret-class-inventory.yaml` (`SUPPRESSION_HMAC_SECRET`), `tests/ci/deletion-phases-235.pg.ci.test.ts` (P2.1–P2.4, P5.3).
