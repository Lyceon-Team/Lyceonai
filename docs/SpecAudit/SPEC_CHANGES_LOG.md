...
SCL-120 | 2026-09-23 | Doc 04B V4.3 §8.3 says `section_total_questions` and `module1_questions` are stored in `scoring_constants` as a convenience cache; Appendix A does not seed them, the scoring function never reads them, and a hand-synced cache with no consumer is a drift source | PROPOSED
Id: `SCL-120` allocated 2026-09-23 as max+1 across every remote branch (max SCL-118 on `origin/calendar` and siblings); second of two sequential allocations this session (SCL-119 is on `claude/e1-exam-deletion`).
