...
SCL-032 | 2026-08-15 | Doc 03B §5.5 step 4 vs implementation scope for INV-03-02 (live exam gate on POST /messages only) | PROPOSED
...
admin absolutely on /api/tutor/_. Doc 07E has no provisions for staff access to tutor data.
The corpus mandates a capability its own role rule forbids.
WAS: Doc 03B §3.1 blocks all non-student roles from /api/tutor/_ (403 role_not_permitted). Doc 03
...
IS: (a) §3.1 stands unchanged for /api/tutor/_. student only. All other roles 403 role_not_permitted.
Already implemented in PR #519. (b) Safety review is a SEPARATE surface outside /api/tutor/_, not
