...
SCL-128 | 2026-09-23 | Doc 06D §6.5 makes the deletion verification a separate job triggered after the T+7 hard-delete completes; nothing built that job, so for five days the record had a table, a writer and a passing test suite and no caller. The verification belongs inside T3 of the deletion itself, and §6.4 step 2's shape validation was never implemented | PROPOSED
Id: `SCL-119` re-derived at the moment of use, 2026-09-23, after `git fetch --all --prune`, by `git grep -hoE 'SCL-[0-9]{3}' <ref> -- docs/SpecAudit/SPEC_CHANGES_LOG.md` across every remote ref, and cross-checked against all eight open PRs (#849, #848, #847, #846, #845, #843, #778, #728 — every head is an in-repo branch already covered by the fetch). Highest allocated anywhere is `SCL-118` (`origin/cleanup`). No collision. Single allocation.
...
Why this surfaced now: `public.verify_deletion_layers` (SCL-119, wired into T3 on 2026-09-23) was run against the 2026-09-23 deletion on 2026-09-25 and returned `fail`, naming `practice_sessions.actor_id` and `practice_session_items.actor_id`. The scan's first real use found a defect nobody was looking for. Had T3 been wired two days earlier, that deletion would have recorded `fail` naming both columns and §6.5 (b) would have paged at deletion time.
