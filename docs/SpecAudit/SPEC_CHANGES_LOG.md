...
IS: the engine shipped between 2026-09-17 and 2026-09-22 and verified in production on 2026-09-22.
  ...
  ENTRY. A question enters review when it is answered incorrectly OR SKIPPED in practice, diagnostic sessions included (ruling 2). A correct practice answer never touches the queue (ruling 13). Entry is automatic and applies to every tier.
  ...
  PICKERS. `GET /api/review/pool` (`review-canonical.ts:1397-1401`) returns counts by section, domain and skill plus past sessions with open entries. Labels are computed at read time in the student's timezone (ruling 20).
  CALENDAR (ruling 14). The calendar reads `review_schedule(student_id, status, queued_at)` and answered `review_session_items`. Review never reads calendar tables. Doc 05F, not "future Doc 04", is the owning document; see SCL-115 to SCL-118.
  TARGET STATE. SM-2 intervals and tutor-in-review, both re-addable by additive migration; neither exists at launch.
Rationale: the old review runtime targeted a schema that no longer existed and returned 503 in production. It was removed (PR #794), the schema was rebuilt (#795), the API rewritten (#799) and the UI built (#807), with fixes in #815 and #824. The rulings that produced this shape are the ruled plan's §3 (`docs/plans/Lyceon_Review_Vertical_Plan.md`), corroborated by `docs/contracts/review-contract.md` and by the migration's own comments. Every clause above traces to one of them: 1 (delete the old review layer), 2 (practice misses AND SKIPS both enter the queue — the ruling that reverses §16's "Skipped questions (served but not submitted) do not enter review"), 3 (the practice writer is a trigger on the item's answered-or-skipped update, atomic with the CAS), 4 (a correct review answer graduates the question, `20260921000000:392`), 5 (backfill existing practice misses and skips through the same queue function, `:593`), 6 (full-length review works automatically once the exam vertical writes to the queue; no stub code, hence the polymorphic `source_session_id`), 7 (oldest first; a wrong or skipped review item requeues at the back), 8 (all tables altered in place, never dropped and recreated), 9 (LISA is out at launch; `used_tutor` stays `false`), 11 (mastery via `applyMasteryEvent` with the item id as the event id), 12 (one row per miss or skip; a new one supersedes the open entry; at most one open per question), 13 (a correct practice answer never touches the queue), 14 (`next_review_at` becomes `queued_at`, with the calendar's two lines in the same migration as the calendar team agreed), 15 (prefill the whole pool; the same question may sit in two open sessions), 16 (a review skip requeues at the back), 17 (inactivity abandonment mirrors practice's 7-day sweep, and abandoned sessions never show in the UI), 18 (a retired question's queue entry is left alone; the servable join excludes it everywhere), 19 (the queue stores no question metadata), 20 (session picker labels are computed at read time), 21 (`review_question_history`, a `security_invoker` view granted to `service_role` only). Ruling 10 belongs to SCL-110. The spec's SM-2 model is not wrong as a target; it is wrong as a description of the launch engine, and §16 currently reads as the latter.
...
Rationale: owner ruling 9 (`docs/plans/Lyceon_Review_Vertical_Plan.md` §3, "LISA is out at launch; `used_tutor` stays `false`"). The danger in this amendment is the obvious edit — striking the tutor-in-review paragraphs entirely — because CR-02B-29 is the only place the answer-withholding architecture is written down, and a future tutor-in-review build that starts from a §21 with no review row would have no rule to implement against. Deferring a surface and deleting its safety rule are different edits; only the first is authorised here.
...
Rationale: rulings 8, 12, 14, 17, 19 and 21 (`docs/plans/Lyceon_Review_Vertical_Plan.md` §3) — ruling 8, "All tables are altered in place, never dropped and recreated", is why `review_schedule` is the same table §8 calls proposed rather than a new one beside it. §8 is the table-level map engineers implement from; three of its five review rows name artefacts that do not exist, and the prefill bullet would lead an implementer to build the wrong thing.
...
Rationale: ruling 14 (`docs/plans/Lyceon_Review_Vertical_Plan.md` §3: "`next_review_at` becomes `queued_at`. Drop the SM-2 columns and `first_missed_session_id`. The calendar changes two lines in the same migration, as the calendar team agreed."), executed in the migration. A seam table that names a renamed column and a column that never existed cannot be implemented from.
...
Rationale: calendar seam brief items H1 to H3, and rulings 11, 15 and 17 (`docs/plans/Lyceon_Review_Vertical_Plan.md` §3). CITATION CORRECTED, 2026-09-22: written first as "rulings 11 and 18" from the brief, before the plan was in the repo. Ruling 18 is about retired questions and supports nothing here; ruling 15 ("the whole pool at creation, or the calendar's size") is what licenses `target_count = size`, and ruling 17 is what gives review the terminal states the `progress` mapping needs. The `unit_id` choice is ruling 11's consequence and not a free one: the review item's id is also the attempt row's id and the mastery event id, so a calendar unit keyed on it joins to the mastery record without a second identifier.
