...
- Question/review handlers mounted by the production server:
  ...
  - `server/index.ts` mounts `/api/review/*` from `server/routes/review-canonical.ts` (the mistake queue; practice's loop, a different pool).
...
- `server/routes/review-errors-routes.ts` and `server/routes/review-session-routes.ts` were DELETED by R1; neither file exists. The canonical review submit route is `POST /api/review/answer` in `server/routes/review-canonical.ts`.
