...
function createChainableMock(returnData: any[] = []) {
  ...
  mock.then = (
    resolve: (value: { data: any[]; error: null }) => any,
...
describe("adaptiveSelector", () => {
  ...
  it("marks bounded degraded fallback when weak-skill source fails", async () => {
    (fetchWeakestSkills as Mock).mockRejectedValueOnce(
      new Error("weakest_skills_query_failed"),
    );
    ...
    expect(result.rationale.sourceWarnings).toEqual([
      "weak_skills_source_failed",
    ]);
    expect(result.rationale.filterPath).toContain(
      "weakest-skills-source-failed",
    );
  });
  ...
  it("does not emit source warning when weak-skill source is healthy", async () => {
    ...
    expect(
      result.rationale.filterPath?.includes("weakest-skills-source-failed"),
    ).toBe(false);
  });
  ...
  it("difficulty randomness is deterministic per seed", async () => {
    ...
    expect(result1.rationale.difficultyPicked).toBe(
      result2.rationale.difficultyPicked,
    );

    const paramsWithDifferentIndex = { ...params, attemptIndex: 4 };
    const result3 = await selectNextQuestionForStudent(params);
    const result4 = await selectNextQuestionForStudent(
      paramsWithDifferentIndex,
    );

    expect(result3.rationale.difficultyPicked).toBe(
      result1.rationale.difficultyPicked,
    );
