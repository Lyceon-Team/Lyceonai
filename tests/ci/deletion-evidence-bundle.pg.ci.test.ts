...
const GUARDIAN = { id: "99999999-0000-4000-8000-000000000009", email: "guardian@evidence.test" };
const STUDENT = { id: "88888888-0000-4000-8000-000000000008", email: "student@evidence.test" };
...
// ── Stripe recorder ──────────────────────────────────────────────────────────
type StripeSub = { id: string; status: string; items: { data: Array<{ id: string }> } };
const stripeState = {
  constructed: 0,
    ...
    return {
      subscriptions: {
        list: async (args: unknown) => {
          stripeState.calls.push({ method: "subscriptions.list", args: [args] });
          return { data: stripeState.owned };
        },
        retrieve: async (id: string) => {
          stripeState.calls.push({ method: "subscriptions.retrieve", args: [id] });
          if (!stripeState.retrieve) throw new Error(`no such subscription ${id}`);
          return stripeState.retrieve;
        },
        cancel: async (id: string, params: unknown) => {
          stripeState.calls.push({ method: "subscriptions.cancel", args: [id, params] });
          if (stripeState.cancelThrows) throw new Error("stripe: service unavailable");
...
function buildAdmin() {
  ...
  return {
    ...
    rpc: async (fn: string, args?: Record<string, unknown>) => base.rpc(fn, args),
    storage: {
...
async function actorIdOf(profileId: string): Promise<string> {
  const r = await pg.query(`SELECT actor_id FROM public.profiles WHERE id = $1`, [profileId]);
...
async function todayUtc(): Promise<string> {
  const r = await pg.query(`SELECT (now() AT TIME ZONE 'utc')::date::text AS d`);
...
describe.skipIf(!PG_AVAILABLE)("deletion evidence bundle — real Postgres", () => {
  beforeAll(async () => {
  ...
  beforeEach(async () => {
    ...
    await pg.query(`DELETE FROM public.profiles WHERE email LIKE '%@evidence.test' OR email LIKE 'deleted_%'`);
    await pg.query(`DELETE FROM auth.users WHERE email LIKE '%@evidence.test'`);
  });
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("C3.1 structural: evidence tables carry no timestamp column, no uuid but the random log_id, no FK outside the bundle, RLS on with zero policies", async () => {
    ...
    const timestampCols = cols.rows.filter((c) => String(c.data_type).startsWith("timestamp"));
    expect(timestampCols).toEqual([]);
    const uuidCols = cols.rows.filter((c) => c.data_type === "uuid" && c.column_name !== "log_id");
    expect(uuidCols).toEqual([]);
    const identityLeak = cols.rows.filter((c) =>
      ["actor_id", "profile_id", "user_id", "email", "created_at", "updated_at"].includes(String(c.column_name)),
    );
    expect(identityLeak).toEqual([]);
    // the log_id default is random, not a sequence
    const logIdDefault = cols.rows.find(
      (c) => c.table_name === "deletion_request_log" && c.column_name === "log_id",
    );
    expect(String(logIdDefault?.column_default)).toBe("gen_random_uuid()");
    expect(cols.rows.some((c) => String(c.column_default ?? "").includes("nextval"))).toBe(false);
  ...
  it("C3.2 cross-universe xmin: no evidence row shares a transaction id with any actor_id-side row", async () => {
    ...
    expect(summary).toEqual({ executedCount: 3, skippedCount: 0, failedCount: 0 });

    const evidence = await evidenceXmins();
    expect(evidence.length).toBeGreaterThanOrEqual(6); // 3 log rows + 3 consent rows
    const actorSide = [
      ...(await xmins(`SELECT xmin::text AS x FROM public.practice_sessions`)),
  ...
  it("C3.3 rank: execution order is ORDER BY profile_id and does not reproduce the evidence side's request order", async () => {
    ...
    const summary = await runExecutor();
    expect(summary).toEqual({ executedCount: 6, skippedCount: 0, failedCount: 0 });
    ...
    const ev = await pg.query(`SELECT subject_email FROM public.deletion_request_log ORDER BY ctid`);
    const evidenceOrder = ev.rows.map((r) => String(r.subject_email));
    const evByLogId = await pg.query(`SELECT subject_email FROM public.deletion_request_log ORDER BY log_id`);
    const evidenceByLogId = evByLogId.rows.map((r) => String(r.subject_email));

    const profileIdOrder = [...USERS].sort((a, b) => (a.id < b.id ? -1 : 1)).map((u) => u.email);
  ...
  it("C3.4 guardian pre-clear: a live student's consent row is not written in the guardian's cascade transaction", async () => {
    ...
    const guardianT2 = await xmins(`SELECT xmin::text AS x FROM public.practice_sessions`);
    expect(guardianT2).toHaveLength(1);
    expect(consentXmin).not.toBe(guardianT2[0]);
    const ledger = await xmins(`SELECT xmin::text AS x FROM public.anonymized_actors`);
  ...
  it("C3.5 cancelled: an in-app cancel logs `cancelled` with a response date and nothing is deleted", async () => {
    ...
    const cancelled = await pg.query(`SELECT public.cancel_account_deletion($1) AS id`, [u.id]);
    ...
    const summary = await runExecutor();
    expect(summary).toEqual({ executedCount: 0, skippedCount: 0, failedCount: 0 });
  ...
  it("C3.6 rolled-back cascade leaves `executing`; the reconciler reverts it to pending, and completes a row whose request is gone", async () => {
    ...
    expect(first).toEqual({ executedCount: 0, skippedCount: 0, failedCount: 1 });
    expect(await profileExists(x.id)).toBe(true);
    const pendingReq = await pg.query(
      `SELECT status FROM public.account_deletion_requests WHERE profile_id = $1`, [x.id],
    );
    ...
    await pg.query(`SELECT public.mark_deletion_log_executing(ARRAY[$1::uuid])`, [logY]);
    expect((await logRow(logY))?.status).toBe("executing");
    await pg.query(`SELECT public.preclear_account_deletion_links($1)`, [y.id]);
    await pg.query(`SELECT public.deidentify_user($1, $2)`, [y.id, `deleted_${y.id}@deleted.lyceon.ai`]);
    const t2 = await pg.query(`SELECT public.complete_and_anonymize_account($1, $2) AS r`, [requestId, y.id]);
  ...
  it("C3.7 no free text from the request row reaches the evidence side", async () => {
    ...
    for (const c of cols.rows) {
      ...
      expect({ table: c.table_name, column: c.column_name, hits: hit.rows[0]?.n }).toEqual({
        table: c.table_name,
  ...
  it("B3.1 Stripe: the person's own subscription is cancelled with { prorate: false } and the billing record survives keyed to the log", async () => {
    ...
    expect(stripeState.calls).toEqual([
      { method: "subscriptions.list", args: [{ customer: "cus_evidence", status: "all", limit: 100 }] },
      { method: "subscriptions.cancel", args: ["sub_evidence", { prorate: false }] },
    ]);
  ...
  it("B3.2 Stripe failure records failed_manual and the deletion still completes", async () => {
    ...
    expect(await profileExists(u.id)).toBe(false);
    expect((await logRow(logId))?.status).toBe("completed");
    const billing = await pg.query(`SELECT final_status, stripe_customer_id FROM public.deletion_billing_record WHERE log_id = $1`, [logId]);
    expect(billing.rows[0]).toEqual({ final_status: "failed_manual", stripe_customer_id: "cus_broken" });
  });
