/**
 * Calendar full-length adapter — launch contract.
 *
 * @spec [Doc-05F_V1.0 §9.1 adapter contract, §9.4 full-length adapter (G-08-02);
 *        Doc_05F_formula_sheet.md §8 item 12, §5A fail-open]
 * | @implemented [2026-09-18]
 *
 * THIS FILE IS THE CONTRACT THE REBUILT full-length ENGINE IS HELD TO.
 *
 * The full-length engine has not shipped. Its adapter is a stub that fails OPEN, and
 ...
 * is ["practice"] at launch so the generator emits no full-length blocks, but a student
...
import { fullLengthAdapter } from "../../server/services/calendar/adapters/stub";
...
describe("full-length adapter — the §9.1 contract", () => {
  ...
  it("(2b) create rejects nothing and resolves — a throw here breaks the launch route", async () => {
    await expect(fullLengthAdapter.create(BLOCK, 1, CTX)).resolves.toBeDefined();
  });

  it("(3) activityUnits returns none, so blocks read 0 / target rather than as an error", async () => {
    await expect(
      fullLengthAdapter.activityUnits(CTX.student_id, "2026-09-18", "America/Chicago"),
    ).resolves.toEqual([]);
  });

  it("(4) progress reports no lifecycle, which keeps the block out of in_progress (§13)", async () => {
    await expect(fullLengthAdapter.progress("any-session-id")).resolves.toBeNull();
  });

  it("(5) nextLaunchSize answers for work still outstanding, and never zero", async () => {
    await expect(fullLengthAdapter.nextLaunchSize(BLOCK, 7)).resolves.toBe(7);
    await expect(fullLengthAdapter.nextLaunchSize(BLOCK, 0)).resolves.toBe(1);
  });

  it("declines every size the same way — the refusal is about the engine, not the ask", async () => {
    for (const size of [1, 5, 30]) {
      const result = await fullLengthAdapter.create(BLOCK, size, CTX);
      expect(result.ok).toBe(false);
