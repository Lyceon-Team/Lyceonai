...
**C11.2** The sweep (Phase 2, landed 2026-09-15). The window has ONE definition: `public.notification_retention_days()` (90). `public.sweep_notification_retention(p_batch_size)` deletes `notification_events` rows with `created_at` older than that window, oldest first, at most `p_batch_size` per call, and returns `(deleted_events, deleted_messages, cutoff)`. Messages and delivery events are removed by the FK cascades — the one DELETE is the whole mechanism, and there is deliberately no second sweep of `notification_messages` (a second delete path would be a second derivation of the same rule). `server/lib/notifications/retention.ts` calls it with `NOTIFICATION_RETENTION_SWEEP_BATCH_SIZE`; `GET /api/internal/notification-retention-sweep` (CRON_SECRET-gated, 404 otherwise) is the scheduled entry point, registered in `vercel.json` (`0 5 * * *`). `pg_cron` is installed, unused, and stays that way. Residual, by design: a delivery event that was never matched to a message (`message_id IS NULL`) has no parent to cascade from and is not swept; the owner decides whether that class needs its own rule.
*Violated if:* a second literal window appears anywhere (the PG suite asserts the sweep body contains no `90`); a delete path for `notification_messages` exists outside the cascade; the sweep deletes a row younger than the window; a call deletes more than `p_batch_size` events; the endpoint answers anything but 404 without the secret.

**C11.3** Every sweep run is logged, zero-deletion runs included: `retention_sweep_completed` with `deletedEvents`, `deletedMessages`, `cutoff` and `batchSize`. A run that deleted nothing and a run that never happened must be distinguishable from the logs alone, because cron registration cannot be read from tooling (owner brief 2026-09-15 Part A2) — the absence of this line over a day is the signal that the job is not running. A failure logs at error and returns 500; it is never swallowed.
*Violated if:* a sweep call returns without emitting the line, or the line omits the count or the cutoff.
