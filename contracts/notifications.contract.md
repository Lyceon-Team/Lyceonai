...
**C0.5** All four direct senders return a `Result`, never throw, and log only ids and a redacted address. The three request-row sends are best-effort after their mutation has committed: a transport failure never fails the request that produced it. The invite has no mutation to protect — the email IS the request — so a transport failure is reported as 503 with a body that is identical for every address (see §2.3 note and `tests/ci/guardian-invite.pg.ci.test.ts` B4.5).
*Violated if:* a consent-request PATCH or a deletion request returns 5xx because mail failed, any of the four senders throws, or the invite route's response differs between an address with an account and one without.

**C0.6** The deletion-completed notice (owner brief 2026-09-15 Part A; SCL-083 PROPOSED) is sent by the cron executor `executeDueDeletions` in `server/lib/account-deletion-execute.ts`, per account, never per batch, and only after `complete_and_anonymize_account` has returned `completed`. The recipient address is read from `profiles.email` in its own PostgREST call BEFORE any mutating step (step 3 `deidentify_user` overwrites it with the `deleted_<id>@` placeholder and the cascade then deletes the row), held in a local for one loop iteration, and **never persisted**: not in a table, not in a queue, not in a log line (only `redactEmail` output is logged). There is **no retry mechanism, by design** — a retry would require persisting the address of a person whose data has just been deleted, which is the one thing this lane must not do. A send failure never fails the deletion (already committed) and never aborts the batch (each iteration is independently guarded). A rolled-back or failed deletion sends nothing. Content is the completion date only: no account details, no name, no progress data, no link, no recovery offer.
*Violated if:* the address read is issued after `deidentify_user` or inside either RPC's transaction; a send happens for a request whose atomic RPC did not return `completed`; a mail failure raises `failedCount` or stops the loop; the raw address appears in any log line or table; a retry loop, queue or outbox is added for this notice; the body contains a URL.
...
**C3.1** `GET /api/notifications` returns only `channel='in_app'` rows. It has two views of those rows, selected by `?archived`: the inbox (`archived_at IS NULL`, the default) and the archive (`archived_at IS NOT NULL`, `?archived=true`). A row is in exactly one view at a time; `archivedAt` is on every item. The same recipient rule and the same keyset cursor apply to both views. (Owner brief 2026-09-15 Part B1: `archived_at` existed but was unreachable from the UI.)
*Violated if:* any item in the feed response corresponds to a row with `channel='email'`; an archived row appears in the inbox view or an unarchived row in the archive view; a value other than the literals `true`/`false` is accepted for `archived` (`z.coerce.boolean` would read `"false"` as true).

**C3.2** Seen, read and archived are three columns and three acts. Opening the /notifications page (or the bell) marks rows SEEN (`POST /mark-all-seen`); READ is explicit — opening an item, its "Mark as read" control, or `POST /mark-all-read`, which stamps `read_at` (and `seen_at` where unset) on the recipient's unread, unarchived in_app rows only and returns the count; archiving is a per-row `PATCH {archived:true}`. Nothing on these surfaces deletes a row.
*Violated if:* opening the page sets `read_at`; mark-all-read touches an archived row or another recipient's row; any of these surfaces issues a DELETE.
...
**C11.2** The sweep (Phase 2, landed 2026-09-15; orphan branch added 2026-09-16). The window has ONE definition: `public.notification_retention_days()` (90). `public.sweep_notification_retention(p_batch_size)` is ONE function, ONE transaction, ONE cutoff derived once from that window, with two branches:
- **Parent path.** `notification_events` rows with `created_at` older than the window, oldest first, at most `p_batch_size` per call. Messages and the delivery events attached to them are removed by the FK cascades — there is deliberately no separate delete of `notification_messages` or of matched delivery events (a second delete path for a row that has a parent would be a second derivation of the same rule). A child's own age never overrides its parent: a matched delivery event whose parent event is inside the window survives whatever its `received_at`.
- **Orphan path.** `notification_delivery_events` rows with `message_id IS NULL` (the §6.5 `unmatched` class: a webhook that never matched, or that arrived after its message was swept) have no parent to cascade from. They are aged on their own `received_at` against the same cutoff, oldest first, at most `p_batch_size` per call. Without this branch the sweep would manufacture orphans — every late webhook for a swept message would persist for ever. These rows carry provider ids, an event type, timestamps and an outcome, no personal data, which is why the same window applies and no separate policy is needed.

The batch bound applies per branch, not as a shared budget: a large event backlog must not starve orphan cleanup. The function returns `(deleted_events, deleted_messages, deleted_orphan_delivery_events, cutoff)`. `server/lib/notifications/retention.ts` calls it with `NOTIFICATION_RETENTION_SWEEP_BATCH_SIZE`; `GET /api/internal/notification-retention-sweep` (CRON_SECRET-gated, 404 otherwise) is the scheduled entry point, registered in `vercel.json` (`0 5 * * *`). `pg_cron` is installed, unused, and stays that way.
*Violated if:* a second literal window appears anywhere, or the sweep body derives the cutoff more than once (the PG suite asserts the body contains no `90` and exactly one call to `notification_retention_days()`); a second sweep function exists; a delete path for `notification_messages` or for matched delivery events exists outside the cascade; either branch deletes a row younger than the window; a matched delivery event is deleted while its parent event is inside the window; a call deletes more than `p_batch_size` events or more than `p_batch_size` orphans; the endpoint answers anything but 404 without the secret.

**C11.3** Every sweep run is logged, zero-deletion runs included: `retention_sweep_completed` with `deletedEvents`, `deletedMessages`, `deletedOrphanDeliveryEvents`, `cutoff` and `batchSize`. A run that deletes zero on BOTH branches still emits the line. A run that deleted nothing and a run that never happened must be distinguishable from the logs alone, because cron registration cannot be read from tooling (owner brief 2026-09-15 Part A2) — the absence of this line over a day is the signal that the job is not running. A failure logs at error and returns 500; it is never swallowed.
*Violated if:* a sweep call returns without emitting the line, or the line omits either count or the cutoff.
...
**C12.4** Every send carries `Reply-To` = `SUPPORT_EMAIL` from `packages/shared/src/support-contact.ts`, set in `transport.ts` — the one place Resend is called — so every message inherits it and no caller overrides it. `NOTIFICATION_FROM_EMAIL` is send-only with no inbox; a reply belongs with support. Deletion notices reply to support as well (an account-recovery question is a support question; `PRIVACY_EMAIL` is the data-rights channel for the privacy policy and data-rights copy, not a reply target). (Owner brief 2026-09-16 Part A.)
*Violated if:* a captured send lacks `reply_to`; the value is a literal rather than the shared constant (the proving test swaps the constant and expects the header to follow); a caller sets its own reply-to; or a `support@`/`privacy@`/`legal@`/`hello@`/`contact@lyceon.ai` literal exists outside `support-contact.ts`.
...
| C3.1 (archive view), C3.2, C9.4 for the page's routes | `tests/ci/notifications-retention-page.pg.ci.test.ts` — inbox excludes archived rows and `?archived=true` returns only them with `archivedAt`; `archived=yes` is 400; PATCH archive moves a row between the views; mark-all-read stamps read+seen on unread inbox rows only (archived and other recipients untouched) and returns the count; mark-all-seen sets `seen_at` and not `read_at`; an outsider reads an empty inbox and archive and marks zero; the archive view paginates on the same cursor |
| C11.1, C11.2, C11.3 | same file — `notification_retention_days()` = 90 and the sweep body carries no literal window; an event at 91 days is deleted with its 3 messages and its delivery event (message count asserted), one at 89 days survives; a known 5-event fixture returns 3/9 with a cutoff within a minute of now−90d; a zero-deletion run emits `retention_sweep_completed` with counts 0 and a cutoff (asserted on the captured log, not the return value); batch size 2 over 5 eligible rows deletes the two oldest per call; batch size 0 raises `22023`. Orphan branch (2026-09-16): an orphan at 91 days is deleted and counted, one at 89 days survives; a matched delivery event at 120 days goes by the cascade with the orphan count at 0 (and stays green when the orphan branch is mutated out — the parent path does that work); a matched delivery event with a 1-day-old parent survives a 200-day `received_at`; a zero-deletion run logs both counts at 0 with the cutoff; batch size 2 over 5 events + 5 orphans deletes the two oldest of each, and `batchFull` is true when only the orphan branch hits its bound; the body derives the cutoff exactly once |
| C12.4 | `tests/ci/notifications.reply-to.contract.test.ts` — `reply_to` present with the unchanged body keys and no tracking fields; the header follows a swapped constant (module mock); `git grep` finds no contact literal outside the constant file; the rendered Privacy Policy page carries `PRIVACY_EMAIL` and never `SUPPORT_EMAIL`, the Terms and Legal hub the reverse |
| C0.4, C0.5 | `tests/ci/notifications.direct-sends.test.ts` — row-derived keys, sender, links, no tracking, Result on failure, all four call sites wired |
