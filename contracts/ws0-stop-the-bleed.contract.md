...
**Citation convention:** every `capture:<line>` citation (in this contract and in the migration) is an **LF-counted physical line** — the number reported by `grep -n` / `git grep -n` / `sed -n` and by the GitHub blob view. The canonical capture (`git hash-object` = `21ecaf668a6d0c04f460334f13a2f9ae25892d7f`) is CRLF-terminated and additionally contains **bare-CR (`\r`) breaks inside quoted cells**, so line-counting is terminator-dependent: `wc -l` (LF) = **8404**, while CR-aware tools (PowerShell `Get-Content`/`Select-String`, and some auditors) split on `\r` as well and report **10826** = 8294 CRLF + 2422 bare-CR + 110 bare-LF — on the *same bytes*. CR-aware tools therefore **must not** be used to verify these citations; use `git grep -n` / `sed -n` (LF) only. The number is a convenience; the **verbatim anchor text** quoted beside each statement is the real proof.
