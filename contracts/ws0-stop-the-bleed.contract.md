...
- `service_role` has `bypasses_rls=true` (capture:8355 — I2). `anon`/`authenticated` have `bypasses_rls=false` (capture:8335,8336). Therefore: enabling RLS and revoking `anon`/`authenticated` grants leaves every service-role read/write path intact, and closes every anonymous/authenticated PostgREST path.
...
Capture state: `questions` `rls_enabled=true` (capture:61); the single SELECT policy is `questions_select_authenticated :: roles={authenticated} :: USING true` (capture:7933). `anon` and `authenticated` hold the full grant set incl. SELECT (capture:3235–3248). Answer-bearing columns: `correct_answer` (NOT NULL, capture:607), `explanation` (capture:592), `answer_text` (capture:591). (The registry's `questions_select_accessible :: roles={anon,authenticated}` does **not** appear in this capture; only `questions_select_authenticated` is present. The apply script drops both names with `IF EXISTS`.)
